...
public class Main {
    public static void main(String[] args) {
        ...
        while (in.hasNextLine()) { // 注意 while 处理多个 case
            ...
            // 每K个字符组成一个新的子串，并用-分隔
            StringBuilder s2 = new StringBuilder(ss[0]);
            int len = s.length();
            for (int i = 0; i < len; i += k) {
                // 如果s的长度还可以被分隔
                s2.append("-");
                if (len - i > k) {
                    s2.append(conversionStr(s.substring(i, i + k)));
                } else {
                    s2.append(conversionStr(s.substring(i)));
                }
            }
     ...
     * 大小写字母的数量相等时，不做转换
     */
    private static String conversionStr(String s) {
        // 计算大写字母的个数,使用正则把所有的大写字母替换成空
        int upperLen = s.length() - s.replaceAll("[A-Z]", "").length();
        // 计算小写字母的个数,使用正则把所有的小写字母替换成空
        int lowerLen = s.length() - s.replaceAll("[a-z]", "").length();
        // 如果大写字母多，则全转为大写
